...
func CreateImage(imagerCfg *Config) error {
	if imagerCfg.EmitProgress {
		installutils.EnableEmittingProgress()
	}

	// Parse Config
	config, err := configuration.LoadWithAbsolutePaths(imagerCfg.ConfigFile, imagerCfg.BaseDirPath)
	logger.PanicOnError(err, "Failed to load configuration file (%s) with base directory (%s)", imagerCfg.ConfigFile, imagerCfg.BaseDirPath)

	// Currently only process 1 system config
	systemConfig := config.SystemConfigs[imagerCfg.SystemConfig]
	...
	err = imagerCfg.buildSystemConfig(systemConfig, config.Disks)
	return err
}

func (cfg *Config) buildSystemConfig(systemConfig configuration.SystemConfig, disks []configuration.Disk) (err error) {
	...
	if isRootFS {
		...
		additionalExtraMountPoints, additionalExtraDirectories, err := setupRootFS(cfg.OutputDir, installRoot)
		if err != nil {
			return err
		}
		...
		diskDevPath, partIDToDevPathMap, partIDToFsTypeMap, isLoopDevice, encryptedRoot, readOnlyRoot, err = setupDisk(cfg.BuildDir, defaultTempDiskName, cfg.LiveInstallFlag, diskConfig, systemConfig.Encryption, systemConfig.ReadOnlyVerityRoot)
		if err != nil {
			return
		}
		...
		err = setupDiskEncryption(&systemConfig, &encryptedRoot, cfg.BuildDir)
		if err != nil {
			return
		}
	...
	setupChrootDir := filepath.Join(cfg.BuildDir, setupRoot)
	...
	if isOfflineInstall {
		...
		err = installutils.ExtractPartitionArtifacts(setupChrootDir, cfg.OutputDir, defaultDiskIndex, disks[defaultDiskIndex], systemConfig, partIDToDevPathMap, mountPointToOverlayMap)
		if err != nil {
			return
		}
		...
		if !isRootFS {
			if disks[defaultDiskIndex].Artifacts != nil {
				input := filepath.Join(cfg.BuildDir, defaultTempDiskName)
				output := filepath.Join(cfg.OutputDir, fmt.Sprintf("disk%d.raw", defaultDiskIndex))
