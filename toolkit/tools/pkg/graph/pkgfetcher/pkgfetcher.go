...
func ResolvePackages(cfg *Config) error {
	...
	if hasUnresolvedNodes(dependencyGraph) {
		err = resolveGraphNodes(dependencyGraph, cfg.InputSummaryFile, cfg.OutputSummaryFile, toolchainPackages, cloner, cfg.OutDir, cfg.DisableUpstreamRepos, cfg.StopOnFailure)
		if err != nil {
...
// to satisfy it.
func resolveGraphNodes(dependencyGraph *pkggraph.PkgGraph, inputSummaryFile, outputSummaryFile string, toolchainPackages []string, cloner *rpmrepocloner.RpmRepoCloner, outDir string, disableUpstreamRepos, stopOnFailure bool) (err error) {
